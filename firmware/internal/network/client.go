...
// SendHeartbeat sends a health/heartbeat update.
...
// SendEvent sends a security or telemetry event.
func (c *Client) SendEvent(ctx context.Context, event map[string]interface{}) (map[string]interface{}, error) {
	return c.doPost(ctx, "http://localhost:8081/api/v1/events", event)
}
...
func (c *Client) GetConfig(ctx context.Context) (map[string]interface{}, error) {
	url := fmt.Sprintf("%s/api/v1/devices/%s", c.baseURL, c.deviceID)
